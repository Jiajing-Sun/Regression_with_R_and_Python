# Chapter 10: Prediction

Companion materials for *Regression with R and Python: Description, Prediction, and Causal Inference*.

## R Markdown workbooks

- `chapter10_workbook.Rmd` is the reference workbook that shows chapter code in textbook order.
- `chapter10_runnable_workbook.Rmd` is the runnable workbook with setup code added so the chapter can usually be executed end-to-end.

## R code boxes

- Example with training and test data
- Cross-validation in R
- Ridge and lasso in R
- Regression tree in R
- Comparison of the different prediction models

## Python code boxes

- No printed code boxes in this language for the current chapter draft.

## Box-level snippets

These files are the QR-code targets for the printed code boxes. The full chapter scripts remain the easiest way to run the chapter end-to-end.

### R box files

- [`R/boxes/box01_example_with_training_and_test_data.R`](R/boxes/box01_example_with_training_and_test_data.R) - Example with training and test data
- [`R/boxes/box02_cross_validation_in_r.R`](R/boxes/box02_cross_validation_in_r.R) - Cross-validation in R
- [`R/boxes/box03_ridge_and_lasso_in_r.R`](R/boxes/box03_ridge_and_lasso_in_r.R) - Ridge and lasso in R
- [`R/boxes/box04_regression_tree_in_r.R`](R/boxes/box04_regression_tree_in_r.R) - Regression tree in R
- [`R/boxes/box05_comparison_of_the_different_prediction_models.R`](R/boxes/box05_comparison_of_the_different_prediction_models.R) - Comparison of the different prediction models

## Notes

- Scripts mirror the current textbook code blocks in the order they appear in the book.
- The repository is aimed at advanced undergraduate and beginning master's students in applied social sciences and econometrics.
- Chapter scripts automatically use the local `data/` folder when one exists, so the printed textbook paths work from the companion repository.
