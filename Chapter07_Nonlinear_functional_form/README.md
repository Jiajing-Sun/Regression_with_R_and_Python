# Chapter 07: Nonlinear functional form

Companion materials for *Regression with R and Python: Description, Prediction, and Causal Inference for Applied Social Science and Econometrics*.

## R Markdown workbooks

- `chapter07_workbook.Rmd` is the reference workbook that shows chapter code in textbook order.
- `chapter07_runnable_workbook.Rmd` is the runnable workbook with setup code added so the chapter can usually be executed end-to-end.

## R code boxes

- Air pollution and weather in R

## Python code boxes

- Air pollution and weather in Python

## Box-level snippets

These files are the QR-code targets for the printed code boxes. The full chapter scripts remain the easiest way to run the chapter end-to-end.

### R box files

- [`R/boxes/box01_air_pollution_and_weather_in_r.R`](R/boxes/box01_air_pollution_and_weather_in_r.R) - Air pollution and weather in R

### Python box files

- [`python/boxes/box01_air_pollution_and_weather_in_python.py`](python/boxes/box01_air_pollution_and_weather_in_python.py) - Air pollution and weather in Python

## Notes

- Scripts mirror the current textbook code blocks in the order they appear in the book.
- The repository is aimed at advanced undergraduate and beginning master's students in applied social sciences and econometrics.
- Chapter scripts automatically use the local `data/` folder when one exists, so the printed textbook paths work from the companion repository.
